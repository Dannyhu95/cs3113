...
float screenShakeSpeed = 40.0;
float screenShakeIntensity = 1.0f;
float screenShakeStart = 0.0f;
float screenShakeDuration = 1.0f;
float animationAValue;
bool done = false;
GLuint mapTexture;
GLuint charTexture;
GLuint fontTexture;
enum GameState { STATE_MAIN_MENU, STATE_GAME_LEVEL, STATE_GAME_LEVEL2, STATE_GAME_LEVEL3, STATE_END};
int state;
int score=0;
...
void draw(ShaderProgram *program, int mapTexture, unsigned char **levelData){
...
void drawPlayer(ShaderProgram *program, int texture, int index){
    float u = (float)(((int)index) % 8) / (float) 8;
    float v = (float)(((int)index) / 8) / (float) 4;
    float spriteWidth = 1.0/(float)8;
    float spriteHeight = 1.0/(float)4;
    GLfloat texCoords[] = {
        u, v+spriteHeight,
        u+spriteWidth, v,
        u, v,
...
bool readHeader2(std::ifstream &stream) {
    string line;
    //mapWidth = -1;
    //mapHeight = -1;
    while(getline(stream, line)) {
...
bool readHeader3(std::ifstream &stream) {
    string line;
    //mapWidth = -1;
    //mapHeight = -1;
    while(getline(stream, line)) {
    ...
    if(mapWidth == -1 || mapHeight == -1) {
        ...
        levelData3 = new unsigned char*[mapHeight];
        for(int i = 0; i < mapHeight; ++i) {
            levelData3[i] = new unsigned char[mapWidth];
        }
        return true;
    }
}
...
bool readLayerData3(std::ifstream &stream) {
    string line;
    while(getline(stream, line)) {
        if(line == "") { break; }
        istringstream sStream(line);
...
void placeEntity2(string type, float x, float y){
    if (type == "player"){
        playerXLVL2 = x;
        playerYLVL2 = y;
        player2XLVL2 = x;
...
void placeEntity3(string type, float x, float y){
    if (type == "player"){
        playerXLVL3 = x;
        playerYLVL3 = y;
        player2XLVL3 = x;
...
bool isSolid2(int x, int y){
    if(levelData2[y][x] == 1 || levelData2[y][x] == 2 || levelData2[y][x] == 3 || levelData2[y][x] == 4 ) {
        return true;
    }
    else
        return false;
...
bool isSolid3(int x, int y){
    if(levelData3[y][x] == 1 || levelData3[y][x] == 2 || levelData3[y][x] == 3 || levelData3[y][x] == 4 || levelData3[y][x] == 5 || levelData3[y][x] == 32 || levelData3[y][x] == 33 || levelData3[y][x] == 34) {
        return true;
    }
    else
        return false;
...
void collisionY(Entity *e){
    ...
    if (mapYBot <= mapHeight-1 && mapYBot >=0 && mapX >= 0 && mapX <= mapWidth-1 && mapYTop <=mapHeight-1 && mapYTop>=0){
        ...
        if (state == STATE_GAME_LEVEL3){
            if(isSolid3(mapX,mapYBot) == true){
                if ((e->position.y-TILE_SIZE/2) <= (-TILE_SIZE* mapYBot)){
                    e->collidBot = true;
                    float penetration = fabs((-TILE_SIZE* mapYBot)-(e->position.y-TILE_SIZE/2));
                    e->position.y = e->position.y + penetration +0.001;
...
void collisionX(Entity *e){
    ...
    if (mapXLeft <= mapWidth-1 && mapXLeft >=0 && mapY >= 0 && mapY <= mapHeight-1 && mapXRight <=mapWidth-1 && mapXRight>=0){
        ...
        if (state == STATE_GAME_LEVEL2){
            if(isSolid2(mapXLeft,mapY) == true){
                if ((e->position.x-TILE_SIZE/2) <= ((TILE_SIZE* mapXLeft)+TILE_SIZE)){
                    e->collidLeft = true;
                    float penetration = fabs((e->position.x-TILE_SIZE/2)-((TILE_SIZE* mapXLeft)+TILE_SIZE));
                    e->position.x = e->position.x + penetration +0.001;
            ...
            if(isSolid2(mapXRight,mapY) == true){
                if ((e->position.x+TILE_SIZE/2) >= (TILE_SIZE* mapXRight)){
                    e->collidRight = true;
                    float penetration = fabs((TILE_SIZE* mapXRight)-(e->position.x+TILE_SIZE/2));
                    e->position.x = e->position.x - penetration - 0.001;
        ...
        if (state == STATE_GAME_LEVEL3){
            if(isSolid3(mapXLeft,mapY) == true){
                if ((e->position.x-TILE_SIZE/2) <= ((TILE_SIZE* mapXLeft)+TILE_SIZE)){
                    e->collidLeft = true;
                    float penetration = fabs((e->position.x-TILE_SIZE/2)-((TILE_SIZE* mapXLeft)+TILE_SIZE));
                    e->position.x = e->position.x + penetration +0.001;
            ...
            if(isSolid3(mapXRight,mapY) == true){
                if ((e->position.x+TILE_SIZE/2) >= (TILE_SIZE* mapXRight)){
                    e->collidRight = true;
                    float penetration = fabs((TILE_SIZE* mapXRight)-(e->position.x+TILE_SIZE/2));
                    e->position.x = e->position.x - penetration - 0.001;
...
void playerEnemyCollision(Entity *e){
    float pTop = e->position.y + TILE_SIZE/2;
    float pBot = e->position.y - TILE_SIZE/2;
    float pLeft = e->position.x - TILE_SIZE/2;
    float pRight = e->position.x +TILE_SIZE/2;
...
void reset(){
    ...
    if(isLevel2== true){
        player.position.x = playerXLVL2;
        player.position.y = playerYLVL2;
        player2.position.x = player2XLVL2;
        player2.position.y = player2YLVL2;
        enemy.position.x = enemyXLVL2;
        enemy.position.y = enemyYLVL2;
    }
    if(isLevel3==true){
        player.position.x = playerXLVL3;
        player.position.y = playerYLVL3;
        player2.position.x = player2XLVL3;
        player2.position.y = player2YLVL3;
        enemy.position.x = enemyXLVL3;
        enemy.position.y = enemyYLVL3;
    }
}
...
void Setup(){
    ...
    charTexture = LoadTexture("characters_3.png");
    ...
    ifstream infile2("NYUCodebase.app/Contents/Resources/gam2.txt");
    string line2;
    while (getline(infile2, line2)) {
        // handle line
        if(line2 == "[header]") {
            if(!readHeader2(infile2)) {
                return;
            }
        } else if(line2 == "[layer]") {
            readLayerData2(infile2);
        } else if(line2 == "[Object Layer 1]") {
            readEntityData2(infile2);
        }
    }
    
    ifstream infile3("NYUCodebase.app/Contents/Resources/game3.txt");
    string line3;
    while (getline(infile3, line3)) {
        // handle line
        if(line3 == "[header]") {
            if(!readHeader3(infile3)) {
                return;
            }
        } else if(line3 == "[layer]") {
            readLayerData3(infile3);
        } else if(line3 == "[Object Layer 1]") {
            readEntityData3(infile3);
        }
    }
    player.friction.x= 2.5;
    player.friction.y= 0.1;
    player2.friction.x= 2.5;
    player2.friction.y= 0.1;
    enemy.velocity.x =15;
}
...
void fixUpdate(float elapsed){
    ...
    player2.collidBot= false;
    player2.collidTop=false;
    player2.collidLeft=false;
    player2.collidRight=false;
    ...
    player2.velocity.x = lerp(player2.velocity.x, 0.0f, elapsed * player2.friction.x);
    player2.velocity.y = lerp(player2.velocity.y, 0.0f, elapsed * player2.friction.y);
    player2.velocity.y += (GRAVITY) * elapsed;
    
    
    player.position.y +=  player.velocity.y * elapsed;
    player2.position.y +=  player2.velocity.y * elapsed;
    //collision y
    collisionY(&player);
    collisionY(&player2);
    
    player.position.x +=  player.velocity.x * elapsed;
    player2.position.x +=  player2.velocity.x * elapsed;
    //collision X
    collisionX(&player);
    collisionX(&player2);
    ...
    playerEnemyCollision(&player2);
    
    if (isLevel1){
    if (player.position.x > mapWidth*TILE_SIZE-TILE_SIZE/2 || player2.position.x > mapWidth*TILE_SIZE-TILE_SIZE/2){
        isLevel1= false;
        state = STATE_GAME_LEVEL2;
    }
    }

    if (isLevel2 == true && isLevel1 == false && isLevel3==false){
        if (player.position.x > mapWidth*TILE_SIZE-TILE_SIZE/2 || player2.position.x > mapWidth*TILE_SIZE-TILE_SIZE/2){
            isLevel2=false;
    ...
    if(lastCollide == false && player.collidBot==false){
        screenShakeValue = 0;
    }
    lastCollide=player.collidBot;
    
    if(lastCollide2 == false && player2.collidBot==false){
        screenShakeValue = 0;
    }
    lastCollide2=player2.collidBot;
    
    
    //reset
    if (player.position.y < -mapHeight*TILE_SIZE-100 || player2.position.y < -mapHeight*TILE_SIZE-100 ){
        reset();
    }
    ...
    if(keys[SDL_SCANCODE_UP]){
        if(player2.collidBot == true){
            Mix_PlayChannel( -1, someSound, 0);
            player2.velocity.y=70;
        }
        
    }
    if(keys[SDL_SCANCODE_LEFT]){
        player2.velocity.x += player2.acceleration.x * elapsed;
        if(player2.acceleration.x==0){
            player2.acceleration.x=-110;
        }
        else{
            player2.acceleration.x= -fabs(player2.acceleration.x);
        }
    }
...
void RenderGameLevel(){
    ...
    program->setProjectionMatrix(projectionMatrix);
    draw(program,mapTexture,levelData);
    modelMatrix2.identity();
    viewMatrix.identity();
    ...
    if(screenShakeValue <= screenShakeDuration/2)
        ...
        animationAValue =mapValue(screenShakeValue, screenShakeDuration, screenShakeDuration / 2, 0.5f, 0.0f);
    viewMatrix.Translate(0.0f, sin(animationAValue * screenShakeSpeed) * screenShakeIntensity, 0.0f);
    /*if(player.velocity.y <= 0 && player.velocity.y >= -0.3){
    viewMatrix.Translate(0.0f, sin(screenShakeValue * screenShakeSpeed) * screenShakeIntensity, 0.0f);
    }*/
    program->setModelMatrix(modelMatrix2);
    program->setViewMatrix(viewMatrix);
    drawPlayer(program, charTexture, 8);
    ...
    modelMatrix4.Translate(player2.position.x, player2.position.y, 0.0f);
    program->setModelMatrix(modelMatrix4);
    drawPlayer(program, charTexture, 0);
    SDL_GL_SwapWindow(displayWindow);
...
void RenderGameLevel2(){
    ...
    program->setModelMatrix(modelMatrix2);
    program->setProjectionMatrix(projectionMatrix);
    draw(program,mapTexture,levelData2);
    modelMatrix2.identity();
    viewMatrix.identity();
    if(isLevel2==false){
        player.position.x = playerXLVL2;
    ...
    program->setModelMatrix(modelMatrix2);
    program->setViewMatrix(viewMatrix);
    drawPlayer(program, charTexture, 9);
    modelMatrix3.identity();
    modelMatrix3.Translate(enemy.position.x, enemy.position.y, 0.0f);
    program->setModelMatrix(modelMatrix3);
    drawEntity(program, mapTexture, 80);
...
void RenderGameLevel3(){
    ...
    program->setModelMatrix(modelMatrix2);
    program->setProjectionMatrix(projectionMatrix);
    draw(program,mapTexture,levelData3);
    modelMatrix2.identity();
    viewMatrix.identity();
    if(isLevel3==false){
        player.position.x = playerXLVL3;
...
void RenderGameOver(){
    ...
    modelMatrix5.Translate(-1.3, 0.5, 0);
    program->setModelMatrix(modelMatrix5);
    program->setProjectionMatrix(projectionMatrix);
    program->setViewMatrix(viewMatrix2);
    DrawText(program, fontTexture, "THE END", 0.4, 0.0);
    modelMatrix5.identity();
    modelMatrix5.Translate(-2.1, -1.0, 0);
    program->setModelMatrix(modelMatrix5);
    DrawText(program, fontTexture, "PRESS SPACE TO CONTINUE", 0.2, 0.0);
    SDL_GL_SwapWindow(displayWindow);
}
